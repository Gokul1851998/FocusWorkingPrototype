...
const MasterLanguage1 = ({ accountName }) => {
  const initialRows = [{ id: 1, language: 'en', master: accountName, languageLabel: "English",toolTip:'',bannerText:'',errorMsg:'' }];
  const { currentTheme } = useTheme();
  const [rows, setRows] = useState(initialRows);
  const [selectedLanguage, setSelectedLanguage] = useState('');
  const [editRowId, setEditRowId] = useState(null);
  const [accountName1, setAccountName1] = useState('');
  const [item,setItem] = useState({toolTip:'',bannerText:'',errorMsg:''})
  ...
  const handleAddLanguage = () => {
    if (selectedLanguage && accountName1) {
      ...
      setAccountName1('');
      setEditRowId(null);
      setItem({ toolTip: '', bannerText: '', errorMsg: '' });
    }
  };
  ...
  const handleEditRow = (row) => {
    ...
    setEditRowId(row.id);
    setItem({ toolTip: row.toolTip, bannerText: row.bannerText, errorMsg: row.errorMsg });
  };
    ...
    <div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <AccountInput1
          label="ToolTip"
          value={item.toolTip}
          onChange={(e) => handleSubmit('toolTip',e.target.value)}
          inputProps={{ ...getLanguageSettings(selectedLanguage) }}
        />
        <AccountInput1
          label="BannerText"
          value={item.bannerText}
          onChange={(e) => handleSubmit('bannerText',e.target.value)}
          inputProps={{ ...getLanguageSettings(selectedLanguage) }}
        />
        <AccountInput1
          label="ErrorMessage"
          value={item.errorMsg}
          onChange={(e) => handleSubmit('errorMsg',e.target.value)}
          inputProps={{ ...getLanguageSettings(selectedLanguage) }}
        />
      </div>
      <Button
        ...
        <Table stickyHeader aria-label="sticky table">
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={headerCellStyle}>BannerText</TableCell>
              <TableCell sx={headerCellStyle}>ErrorMessage</TableCell>
...
export default MasterLanguage1;
