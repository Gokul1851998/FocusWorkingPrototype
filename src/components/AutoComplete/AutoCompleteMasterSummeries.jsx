...
import { Popup } from "../../containers/Home/Master/Account/AccountMaster/Popup";
 ...
 export default function AutoCompleteMasterSummeries ({
    value,
    onChangeName,
    key,
    label,
    ...
    const handleAccess = (item) =>{
        access(item); 
        setSuggestion([...suggestion ,{sName:item.name}])
    }
  ...
  return (
    ...
    renderInput={(params) => (
            ...
            InputProps={{
                ...
                endAdornment: (
                  <React.Fragment>
                      ...
                      <div>
      <i onMouseOver={handleClick} style={{ marginRight: "8px" }} className="fa-solid fa-ellipsis-vertical"></i>
      <Menu
        anchorEl={anchorEl}
      ...
      >
        {value ? (
  <>
    <MenuItem onClick={handleEdit}>Edit Layout</MenuItem>
    <MenuItem onClick={handleClose}>Delete Layout</MenuItem>
  </>
) : (
  <MenuItem onClick={handleCreate}>Create Layout</MenuItem>
)}      
        
      </Menu>

      {/* Render TablePopup if showTablePopup is true */}
      {showTablePopup && <Popup value={value} onClose={() => setShowTablePopup(false)} access={handleAccess} />}
    </div>
