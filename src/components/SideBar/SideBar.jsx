...
import { Avatar, Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Menu, MenuItem, Switch, Tooltip } from "@mui/material";
...
export default function SideBar() {
  ...
  // State for MenuType
  const [selectedMenuType, setSelectedMenuType] = useState("Classic");
  const [defaultMenuType, setDefaultMenuType] = useState("Classic");
  ...
  // MenuType useEffect
  useEffect(() => {
    // Load default MenuType from localStorage
    const storedMenuType = localStorage.getItem("defaultMenuType");
    if (storedMenuType) {
      setDefaultMenuType(storedMenuType);
      setSelectedMenuType(storedMenuType);
    }
  }, []);

  // Function to handle MenuType change
  const handleChangeMenuType = (event) => {
    setSelectedMenuType(event.target.value);
    setOpenPopup(true);
  };

  // Function to handle popup confirmation
  const handleConfirmPopup = () => {
    localStorage.setItem("defaultMenuType", selectedMenuType);
    setDefaultMenuType(selectedMenuType);
    setOpenPopup(false);
  };
  ...
  const handleClosePopup = () => {
    setOpenPopup(false);
    // Reset to the previous defaultMenuType if the user cancels
    setSelectedMenuType(defaultMenuType);
  };
    ...
    <>
        ...
        <Toolbar sx={{ justifyContent: "space-between" }}>
          ...
          <div>
            ...
            >
              ...
              <Box sx={{ marginLeft: 2 }}>
                {/* Add language selection dropdown or list here */}
                <select value={selectedMenuType} onChange={handleChangeMenuType}>
      ...
      <Drawer variant="permanent" open={open}>
       ...
       <Dialog open={openPopup} onClose={handleClosePopup}>
        <DialogTitle>Set Default MenuType</DialogTitle>
        <DialogContent>
          <DialogContentText>Do you want to set {selectedMenuType} as your default MenuType?</DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button variant="contained" onClick={handleClosePopup} color="primary">
            Cancel
          </Button>
          <Button variant="contained" onClick={handleConfirmPopup} color="primary">
            Confirm
          </Button>
        </DialogActions>
      </Dialog>
